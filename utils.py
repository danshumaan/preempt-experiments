    ...
    def encrypt_names(self, inputs: List[str], **kwargs) -> Union[List[str], Union[List[str],List[str]], Dict[str,str]]:
        ...
        if use_fpe:
            ...
            for country in countries: 
                first_names = first_names + self.nd.get_top_names(n=1000,gender='M',country_alpha2=country)[country]['M']
            for country in countries: 
                last_names = last_names + self.nd.get_top_names(n=1000,country_alpha2=country,use_first_names=False)[country]
            ...
            first_names = first_names[:min(-len(set(input_first_names))-1,-2)] + list(set(input_first_names)) + [first_names[-1]]
            last_names = last_names[:min(-len(set(input_last_names))-1,-2)] + list(set(input_last_names)) + [last_names[-1]]
